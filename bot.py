...
FREE_MODELS = {
    ...
    "llama-3.2-90b-vision-preview": "👁️ Llama 90B Vision"  # Модель с поддержкой зрения
}
...
# Хранилище истории диалогов (в памяти, но будет сохраняться в файл)
...
def get_context_messages(user_id: int, max_messages: int = 10) -> list:
    ...
    history = get_user_history(user_id)
    # Берём последние max_messages * 2 (так как каждое сообщение пользователя + ответ ИИ)
...
async def handle_photo(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Если есть текст с фото + возможно пользователь добавил текст
        user_question = message.caption or "Что написано на этом изображении?"
        
        # Формируем запрос
        full_prompt = f"Текст с фото: {extracted_text}\n\nВопрос пользователя: {user_question}"
        ...
        # Сохраняем в историю
        add_to_history(user_id, "user", f"[Фото] {user_question} | Текст: {extracted_text[:200]}..." if len(extracted_text) > 200 else f"[Фото] {user_question}")
